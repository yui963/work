...
const foldersNamePath: string = path.join(SOURCE_STRAGE, KokokonoLabsLogFormatter.FOLDERS, `FoldersName.txt`);
...
async function writeProgressJson() {
    const jsonPath: string = path.join(process.cwd(), "output", "stdntProgress.json");
...
async function calcSession() {
    ...
    const result: number[] = new Array(progressInfoArray[1].progress.length).fill(0);
