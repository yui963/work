 ...
 */
async function formatTxtFileName(
  inputDir: string,
  outputDir: string,
  type: string
): Promise<void> {
 ...
 */
async function copyDirIfNotExist(
  sourceDir: string,
  destinationDir: string
): Promise<void> {
 ...
 */
async function unzipFileOrCopyDirOnList(
  sourceDir: string,
  destinationDir: string,
  ...
  try {
    for (const timestamp of timestampList) {
      ...
      if (await isFileExists(sourceZipDirPath)) {
        ...
        const sourceTimeDirPath: string = join(
          sourceDir,
          timestamp,
          "workspace"
        );
...
async function formatOneProject(
  ...
  // 入力パス
  const runResultsInPath: string = join(
    inputBasePath,
    KokokonoLabsLogFormatter.RUN_LOG_DIR
  );
  const testResultsInPath: string = join(
    inputBasePath,
    KokokonoLabsLogFormatter.TEST_LOG_DIR
  );
  const editActivityInPath: string = join(
    inputBasePath,
    KokokonoLabsLogFormatter.EDIT_LOG_DIR
  );
  const wsHistoryInPath: string = join(
    inputBasePath,
    KokokonoLabsLogFormatter.WORKSPACE_HISTORY_DIR
  );
  // 出力パス
  const debugSessionsOutPath: string = join(
    outputBasePath,
    KokokonoLabsLogFormatter.DEBUG_SESSION_DIR
  );
  const testResultsOutPath: string = join(
    outputBasePath,
    KokokonoLabsLogFormatter.TEST_LOG_DIR
  );
  const editActivityOutPath: string = join(
    outputBasePath,
    KokokonoLabsLogFormatter.EDIT_LOG_DIR
  );
  const wsHistoryOutPath: string = join(
    outputBasePath,
    KokokonoLabsLogFormatter.WORKSPACE_HISTORY_DIR
  );
  ...
  try {
    await fs.copyFile(
      join(runResultsInPath, "runResultsLogger.txt"),
  ...
  try {
    await fs.copyFile(
      join(runResultsInPath, "debugLogger.txt"),
...
async function formatAsync() {
  ...
  const foldersNamePath: string = join(
    drivePath,
  ...
  for (const studentID of studentIDList) {
    ...
    await getSubdirectories(studentIDPath).then(async (miniCV: string[]) => {
      ...
      for (const miniCV of filteredData) {
        ...
        const inputBasePath: string = join(
          miniCVPath,
          KokokonoLabsLogFormatter.BASE_DIR
        );
        const outputBasePath: string = join(
          miniCVPath,
          KokokonoLabsLogFormatter.BASE_FORMAT_DIR
        );
