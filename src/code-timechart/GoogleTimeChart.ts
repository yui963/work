...
async function createGoogleTimeChartDataForEdit(state: State, event: EditEvent): Promise<GTimeChartData | undefined> {
...
async function appendGoogleTimeChartDataForTest(state: State, event: TestEvent, timeChartDataList: GTimeChartData[]) {

    {
        let barLabel = Math.round(event.passRatio) + "%"
        barLabel += "( failed: " + event.failedCase.length + " of " + event.testingCase.length + ")"
    ...
    for (const treeNode of event.testTree) {
        ...
        const failedTestNum = treeNode.testMethodNameList.filter(item => event.failedCase.includes(item)).length;
        const passRatio = 100 * ((treeNode.testMethodNameList.length - failedTestNum) / treeNode.testMethodNameList.length)
        let barStr = Math.round(passRatio) + "%" + "( failed: " + failedTestNum + " of " + treeNode.testMethodNameList.length + " )"
        const timeChartData: GTimeChartData = {
            state: state,
        ...
        for (const testCase of treeNode.testMethodNameList) {
            const rowLabel = testClassName + "->" + testCase.replace(treeNode.testClassName.toString(), "")
            let barStr = "PASS"
...
async function createGoogleTimeChartDataForRun(state: State, event: RunEvent): Promise<GTimeChartData | undefined> {
...
async function createGoogleTimeChartDataForWS(state: State, event: WSEvent): Promise<GTimeChartData | undefined> {
...
async function convertGoogleTimeChartData(stateList: State[], options: any): Promise<String> {
    ...
    for (const state of stateList) {
        ...
        if (state.type == "edit") {
            const event: EditEvent = state.info as EditEvent
            const timeChartData = await createGoogleTimeChartDataForEdit(state, event)
            if (timeChartData != null) {
                timeChartDataList.push(timeChartData)
                timeChartDataList.push(await getSummaryData(timeChartData))
            }
 ...
 * @returns 
 */
async function convertGoogleTimeChartString(timeChartDataList: GTimeChartData[], options: any): Promise<String> {
    ...
    for (const t of timeChartDataList) {
        ...
        if (options.timeFormat == "estimate") {
            ...
            data += "new Date(" + t.beginDate.getFullYear() + "," + t.beginDate.getMonth() + "," + t.beginDate.getDay() + "," + t.beginDate.getHours() + "," + t.beginDate.getMinutes() + "," + t.beginDate.getSeconds() + "), "
            data += "new Date(" + t.endDate.getFullYear() + "," + t.endDate.getMonth() + "," + t.endDate.getDay() + "," + t.endDate.getHours() + "," + t.endDate.getMinutes() + "," + t.endDate.getSeconds() + "), "
        }
        data += "]"
    }
    data += "\n]"
...
export async function createGoogleTimeChart(header: string, stateList: State[], options: any, templatePath: string, chartFilePath: string) {
    try {
        const heaerReplacePattern = "##%%$$HEADER$$%%##";
        const dataReplacePattern = "##%%$$DATA$$%%##";
        const chartData: String = await convertGoogleTimeChartData(stateList, options);
