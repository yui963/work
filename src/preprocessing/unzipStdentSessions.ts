...
const SOURCE_STRAGE: string = 'F:';
const DEST_STRAGE: string = 'G:';

const DEST_FOLDERSNAME: string = 'All';

const foldersNameDir: string = path.join(KokokonoLabsLogFormatter.FOLDERS, `FoldersName${DEST_FOLDERSNAME}.txt`);
const foldersNamePath: string = path.join(DEST_STRAGE, foldersNameDir);
let sidList: string[] = fs.readFileSync(foldersNamePath, 'utf-8').split('\r\n').filter(Boolean); // Mac は "\n"
if (sidList[0].length > 9) {
    sidList = fs.readFileSync(foldersNamePath, 'utf-8').split('\n').filter(Boolean);
}
    ...
    try {
        ...
        for (let i = 0; i < 14; i++) {
            ...
            if (hoge === 1) {
                ...
                console.warn(`Warn: getMustUnzip(): ${hoge}: check file ${sid}/${right.session[i]}`);
            };
        }
...
async function updateUnzipedDataToJson(output: string) {
    ...
    for (const sid of sidList) {
        const sidPath: string = path.join(DEST_STRAGE, KokokonoLabsLogFormatter.UNZIPS, sid);
...
async function main() {
    ...
    const progressJsonPath: string = path.join(process.cwd(), 'output', 'stdntProgress.json');
    const unzipedJsonPath: string = path.join(process.cwd(), 'output', 'unziped.json');
    ...
    for (const item of unzipTarget) {
        ...
        for (const zipfile of zipfileList) {
            for (const target of unzipTarget) {
                if (item.studentID === target.studentID && getZipSessionNo(zipfile, target.session)) {
                    const zipPath: string = path.join(sourceSidPath, zipfile);
                    const destPath: string = path.join(DEST_STRAGE, KokokonoLabsLogFormatter.UNZIPS, target.studentID);
                    try {
